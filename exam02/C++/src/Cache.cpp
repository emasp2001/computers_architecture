...
class Cache {
  ...
  // If alloc is true, it means that the block is being allocated.
  // If alloc is false, it means that the block is being freed.
  void update_cache(int address) {
    int set = this->get_set(address);
    int tag = this->get_tag(address);
    int offset = this->get_offset(address);
    int block = this->get_block(address);
    int block_address = this->get_block_address(address);
    int block_tag = this->get_block_tag(address);
    int block_offset = this->get_block_offset(address);
